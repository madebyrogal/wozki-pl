...
abstract class sfAction extends sfComponent
{
  ...
  {
    //Sprawdzanie uprawnień do modułu dla CMS'a (backend)
    if( sfConfig::get('sf_app') == 'backend'){
      if( $this->getUser()->isAuthenticated() ){
        $modulePermission = module_cmsTable::getModulePermission( $this->getModuleName() );
        if( $modulePermission ){
          ...
          if( !$authenticatedModule ){
            throw new Exception('You have not permission to this module');
          }
        }
      }
      
      //Czyszczenie używanych modułów common i ich rodziców ale tylko dla zwykłych modułów
      //var_dump($this->moduleName);die;
      $commonModules = array('text', 'media', 'attachment', 'meta_tag', 'attribute_def');
      $moduleName = array('textModule', 'mediaModule', 'attachmentModule', 'meta_tagModule', 'attributeDefModule');
      $parentName = array('textParent', 'mediaParent', 'attachmentParent', 'meta_tagParent', 'attributeDefParent');
      if(!in_array($this->moduleName, $commonModules)){
        foreach ($moduleName as $val){
          $this->getUser()->getAttributeHolder()->remove($val);
        }
        foreach ($parentName as $val){
          $this->getUser()->getAttributeHolder()->remove($val);
        }
      }
    }
   ...
   * @param type $page
   */
  protected function setMeta($page){
    $this->getResponse()->addMeta('title', $page->getMetaTitle());
